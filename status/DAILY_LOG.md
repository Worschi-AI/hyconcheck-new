...
## 2026-09-17 – Etappe 8: BL-002.1 – Systematische Literaturrecherche Cluster A–E (Contradiction Detection, NLI, semantische Konsistenz)

- **Arbeitspaket:** AP1
- **Fragestellung / Ziel:** Klärungsziele Z1–Z3, Z10, Z11 des Rechercheprotokolls für die Cluster A–E: Stand der Technik zu Contradiction Detection, NLI, Document-level NLI, Cross-document Contradiction Detection, Semantic Consistency Checking; Benchmarks; Arbeiten, die die Lückenannahme stützen oder einschränken.
- **Tätigkeit (automatisiert, werkzeuggestützt, unter Anweisung des Projektverantwortlichen):** 28 API-Suchläufe (Semantic Scholar Bulk-API, arXiv-API) mit den Protokollstrings PS-A–PS-E, ES-01–ES-07, ES-23, ES-24 in systemspezifischer Syntax (`-v2`) sowie dokumentierten Ableitungen (`-v3`); 9 Browser-Suchläufe (Google Scholar, ACL-Anthology-Suche); 3 Backward-Snowballing-Läufe; Screening Stufe 1 (ca. 1.080 Titel), Stufe 2/3 (52 Quellen); bibliografische Verifikation aller Registereinträge über Semantic-Scholar-Batch-API, ACL-Anthology-Seiten, arXiv und doi.org; 34 Volltexte (PDF) abgerufen und textextrahiert, 12 Quellen nur Abstract-Basis (paywalled/gesperrt); Extraktion nach Protokoll Abschnitt 13, qualitative Bewertung nach Abschnitt 14; Zwischenauswertung erstellt; Prüfskript erweitert. Nicht abgefragt: IEEE Xplore, ACM DL, SpringerLink, ScienceDirect (Ressourcenentscheidung, vorgesehen für BL-002.2 ff.); DBLP-API beim Test HTTP 500.
- **Artefakte:** `references/LITERATURE_SEARCH_LOG.md` (41 Einträge), `references/SOURCES.md` (52 Einträge: 47 aufgenommen, 5 ausgeschlossen), `references/LITERATURE_MATRIX.csv` (47 Zeilen), `references/reviews/REVIEW_A_E_CONTRADICTION_NLI.md`, `references/README.md`, `planning/BACKLOG.md` (BL-002.1–BL-002.5), `status/CURRENT_STATUS.md`, `status/DAILY_LOG.md`, `docs/RESEARCH_DESIGN.md` (Verweis), `tests/check_repo_conventions.py`, `tests/README.md`
- **Ergebnis und Prüfungen:** Verteilung der Aufnahmen nach Primärcluster A 20 / B 8 / C 5 / D 5 / E 9; 42 peer-reviewed, 5 Preprints; 18 Quellen mit teilweiser (17) bzw. weitgehender (1: FactTrack, SRC-0035) Vorwegnahme von HyConCheck-Komponenten; `gap_evidence`: 10 stützend, 14 einschränkend, 23 neutral. `py tests/check_repo_conventions.py` bestanden (inkl. neuer Registerprüfungen und Negativtests mit vollständiger Rücknahme); `git diff --check` bestanden; `python -m pytest` / `ruff check .` weiterhin nicht anwendbar (siehe `tests/README.md`). Für die PDF-Textextraktion wurde das Python-Paket `pypdf` in der Arbeitsumgebung installiert (kein Repository-Artefakt).
- **Interpretation (vorläufig, nur Cluster A–E):** Dokument- und dokumentübergreifende Widerspruchserkennung wird in den geprüften Arbeiten überwiegend durch Satz-/Chunk-NLI mit Aggregation, LLM-Prompting oder synthetische Benchmarks adressiert; graphbasierte und hybride Ansätze existieren; zeitliche Gültigkeit, Status und Versionen werden – außer konzeptuell in CST und in FactTrack (Erzähldomäne) – nicht modelliert. Diese Beobachtung muss in den Clustern F–P geprüft werden.
- **Einschränkungen und Fehlschläge:** Strings ES-03-v2, ES-04-v2, ES-24-v2 lieferten in der Bulk-Suche fachfremde Fenster (dokumentiert, Ableitungen erstellt); Google-Scholar-Seite 3 renderte unvollständig; 4 Volltexte nicht abrufbar (MDPI ×2, NLDL, ACL L04-1239), 8 paywalled; SRC-0003 und SRC-0051 mit EX-7 ausgeschlossen. Keine Hypothese bewertet, keine Forschungslücke festgestellt, kein Obertyp operationalisiert.
- **Entscheidung / nächster Schritt:** BL-002.1 abgeschlossen; BL-002 bleibt offen (1 von 5 Tranchen); AP1 offen. Nächste fachliche Etappe: BL-002.2 – Cluster F–I (Entity Resolution, Temporal Reasoning, Temporal Knowledge Graphs, Status / State Transition Modeling) mit neu gefassten Strings ES-03/ES-04 und Einbeziehung von IEEE/ACM/Springer/ScienceDirect.
- **Präzisierung nach unabhängigem Qualitäts-Audit (17.09.2026, vor Commit-Amend):** Pipe-Zeichen im Suchprotokoll escaped; SRC-0044 als LLM-basierter Ansatz mit explizit eingebundenem Domänenwissen (nicht als symbolisch-neuronales Hybridverfahren) geführt; Benchmark-Aussage auf im Volltext geprüfte Benchmarks begrenzt; 12 Quellen mit Status `abstract-only` (Protokoll V1.1) statt `fulltext`; SRC-0028 und SRC-0034 über ACL Anthology nachverifiziert (Findings of EMNLP 2022 bzw. 2025, DOIs ergänzt); SRC-0041 und SRC-0043 über IEEE Xplore nachverifiziert (Abstract identisch), SRC-0046 (ScienceDirect) wegen Bot-Prüfung nicht zugänglich – Abstract-Basis beibehalten; Prüfskript um entsprechende Prüfungen erweitert. Keine neue Quelle, keine Entfernung, keine Bewertung.
- **Commit-/PR-Referenz:** Branch `work/bl-002-literature-search` (Basis `main` @ `75604eb`), Commit „Literaturrecherche zu Contradiction und NLI durchführen“ (nach Audit per `--amend` korrigiert; SHA in der Git-Historie); PR noch nicht erstellt.
